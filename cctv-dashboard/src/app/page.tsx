...
export default function Dashboard() {
  ...
  // Only use user cameras - no default cameras
  const allCameras = userCameras;
      ...
      <div className="flex h-[calc(100vh-140px)]">
        ...
        <div className="flex-1 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-6 h-[calc(100%-60px)]">
                ...
                <div className="aspect-video bg-gray-800 rounded-xl border-2 border-green-500/50 hover:border-green-500 transition-all duration-300 shadow-lg shadow-green-500/10 hover:shadow-green-500/20 overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-4">
                    <div className="flex items-center justify-between text-green-400 font-mono text-sm">
                      <div>
                        <div className="font-bold">{camera.name}</div>                        <div className="text-xs text-green-400/70">
            ...
            ))}            {/* Add Camera Component - Always show when less than 8 cameras */}
            {allCameras.length < 8 && (
              <div
                onClick={() => setShowCameraManager(true)}
              ...
              >
                <div className="text-center text-green-400/70 group-hover:text-green-400 transition-colors">
                  ...
                  <div className="text-sm">Add webcam, URL, or RTSP stream</div>
                </div>
              </div>
            )}

            {/* Show welcome message when no cameras are added */}
            {allCameras.length === 0 && (
              <div className="col-span-full flex items-center justify-center p-12">
                <div className="text-center text-green-400/70">
                  <div className="text-6xl mb-4">📹</div>
                  <div className="font-mono font-bold text-xl mb-2">
                    WELCOME TO CCTV WEAPON DETECTION
                  </div>
                  <div className="text-sm mb-4">
                    Start by adding your first camera feed
                  </div>
                  <button
                    onClick={() => setShowCameraManager(true)}
                    className="px-6 py-3 bg-green-600 hover:bg-green-700 text-black font-semibold rounded-lg border border-green-400 transition-all duration-200 transform hover:scale-105"
                  >
                    ➕ ADD FIRST CAMERA
                  </button>
                </div>
